...
public class Sender implements Runnable {
    ...
    void run(long now) {
        ...
        // 如果与node没有连接(如果可以连接,同时初始化连接)
        // 就证明该node暂时不能发送数据,暂时移除该node
        ...
        // 返回该node 对应的所有可以发送的RecordBatch组成的batchs(key 是node.id)
        // 并将RecordBatch从对应的queue中移除
     ...
     * 发送produce请求
     * 将batches中leader为同一个node的所有RecordBatch放在一个请求中进行发送
     */
