...
public final class RecordAccumulator {
     ...
     *                       <p>
     *                       Producer会先将record写入到buffer中,当达到一个batch.size的大小时,再唤起sender线程去发送
     *                       RecordBatch(第五步),这里先详细分析一下Producer是如何向buffer中写入数据的.
     *                       Producer是通过RecordAccumulator实例追加数据,RecordAccumulator模型如下
     *                       一个重要的变量就是ConcurrentMap<TopicPartition,Deque<RecordBatch>> batches
     *                       每个TopicPartition都会对应一个Deque<RecordBatch> ,当添加数据时,会向其topic-partition对应
     *                       的这个queue最新创建的一个RecordBatch中添加record,而发送数据时,则会先从queue中最老的那个
     *                       RecordBatch开始发送
     *                       <p>
     *                       <p>
     *                       向accumulator添加一条record,并返回添加后的结果:
     *                       public final FutureRecordMetadata future;
     *                       public final boolean batchIsFull; batch 是否满的标志
     *                       public final boolean newBatchCreated; 新 batch 是否创建
     *
     *
     *
     *
     *                       1. 获取该topicParition对应的queue,没有的话会创建一个空的queue
     *                       2. 向queue中追加数据,先获取queue中最新加入的那个RecordBatch,如果不存在或者存在但是剩余空间
     *                          不足以添加本条record则返回null,成功写入的话直接返回结果,写入成功
     *                       3. 创建一个新的RecordBatch,初始化内存大小根据xxx来确定,防止单条record过大的情况
     *                       4. 向新建的RecordBatch写入record,并将RecordBatch添加到queue中,返回结果,写入成功
     *
     */
    public RecordAppendResult append(TopicPartition tp,
        ...
        try {
            ...
            // 1. 获取tp对应的queue(每个topicPartition对应一个queue)
            Deque<RecordBatch> dq = getOrCreateDeque(tp);
            // 在对一个queue进行操作时,会保证线程安全
            synchronized (dq) {
                if (closed)
                    throw new IllegalStateException("Cannot send after the producer is closed.");
                RecordAppendResult appendResult = tryAppend(timestamp, key, value, callback, dq);
                if (appendResult != null)
                    // 这个topicPartition已经有记录了????????????
                    return appendResult;
            }

            // we don't have an in-progress record batch try to allocate a new batch
            // 2. 为topic-partition创建一个新的RecordBatch,需要初始化相应的RecordBatch
            // 要为其分配的大小是: max(batch.size,加上头文件的本条消息的大小)
            int size = Math.max(this.batchSize, Records.LOG_OVERHEAD + Record.recordSize(key, value));
            log.trace("Allocating a new {} byte message buffer for topic {} partition {}", size, tp.topic(), tp.partition());
            // 3. 给这个RecordBatch初始化一个buffer
            ...
            synchronized (dq) {
                ...
                if (closed)
                    throw new IllegalStateException("Cannot send after the producer is closed.");
                RecordAppendResult appendResult = tryAppend(timestamp, key, value, callback, dq);
                // 4. 如果发现这个queue已经存在,那么就释放这个已经分配的空间
                if (appendResult != null) {
                    // Somebody else found us a batch, return the one we waited for! Hopefully this doesn't happen often...
                    free.deallocate(buffer);
                    return appendResult;
                }
                // 5. 给topicPartition创建一个RecordBatch
                MemoryRecordsBuilder recordsBuilder = MemoryRecords.builder(buffer, compression, TimestampType.CREATE_TIME, this.batchSize);
                RecordBatch batch = new RecordBatch(tp, recordsBuilder, time.milliseconds());
                // 6. 向新的RecordBatch中追加数据
                FutureRecordMetadata future = Utils.notNull(batch.tryAppend(timestamp, key, value, callback, time.milliseconds()));
                // 7. 将RecordBatch添加到对应的queue中
                dq.addLast(batch);
                // 8. 向未ack的batch集合添加这个batch
                incomplete.add(batch);
                // 如果dp.size()>1 就证明这个queue有一个batch是可以发送了
